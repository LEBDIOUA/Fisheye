...
class ModalLightBox{
	...
	static construireLightBox() {
		...
		lbContent.innerHTML = `
				<div class = 'fermerLightBox' tabindex = '0' role= 'button' aria-label = 'Cliquez ou appuiyez pour fermer le lightBox'><i class='fa-solid fa-xmark'></i></div>
				<div class = 'precedent' tabindex = '0' role= 'button' aria-label = 'Cliquez ou appuiyez pour afficher le média précedente'><i class='fa-solid fa-angle-left active'></i></div>
				<div class='lightBoxContent'>
				</div>
				<h2></h2>
				<div class = 'suivant' tabindex = '0' role= 'button' aria-label = 'Cliquez ou appuiyez pour afficher le média suivante'><i class='fa-solid fa-chevron-right active'></i></div>
        `;
	...
	chargerLigthBox() {
		...
		if(this.listeMedias[this.position].getMediaFactory(this.photograph).includes('video')){
			...
			lightBoxContent.querySelector('.media').setAttribute('aria-label', this.listeMedias[this.position].getTitre)
		}
	...
	afficherLightBox() {
		...
		if (this.position === 0) {
			this.btnPrecedent.classList.add('desactive');
			this.btnPrecedent.classList.remove('active');
			...
			this.btnSuivant.classList.add('desactive');
			this.btnSuivant.classList.remove('active');
	...
	controlerVideo(){
		...
		// this.modal.querySelector(".volume").addEventListener('click', this.volumeVideo.bind(this));
	...
	mediaPrecedent() {
		...
		if (this.position === 0) {
			...
			this.btnSuivant.removeAttribute('aria-label');
			this.btnSuivant.setAttribute('aria-label', 'Cliquez ou appuiyez pour afficher le média suivante');
		}
	}
	...
	mediaSuivant() {
		...
		if (this.position === 1) {
			...
			this.btnPrecedent.removeAttribute('aria-label');
			this.btnPrecedent.setAttribute('aria-label', 'Cliquez ou appuiyez pour afficher le média précedente');
		} else if (this.position === this.listeMedias.length - 1) {
	...
	lirePauseVideo(){
		...
		if (video.paused || video.ended) {
			...
			btn.setAttribute('aria-label', 'Lecture/Pause, Cliquez ou appuiyez sur la touche Entrée pour mettre en pause la vidéo');
		} else {
			video.pause();
			btn.removeAttribute('aria-label');
			btn.setAttribute('aria-label', 'Lecture/Pause, Cliquez ou appuiyez sur la touche Entrée pour lire la vidéo');
		}
	}
